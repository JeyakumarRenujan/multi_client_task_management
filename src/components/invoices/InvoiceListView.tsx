    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-whatsapp-teal hover:from-emerald-700 hover:to-whatsapp-dark text-white text-sm font-bold shadow-md shadow-emerald-700/25 active:scale-95 transition-all self-start sm:self-auto cursor-pointer"
        >
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{inv.clientName}</div>
                      {linkedProject && (
                        <div className="inline-flex items-center gap-1.5 mt-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-xs font-medium text-slate-600 dark:text-slate-300">
                    ...
                    <td className="py-3.5 px-4 max-w-[220px]">
                      <div className="truncate text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300">
                        {firstDescription}
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        <select
                          value={inv.status}
                          onChange={e => updateInvoiceStatus(inv.id, e.target.value as InvoiceStatus)}
                          className={`text-xs font-extrabold px-2.5 py-1 rounded-lg uppercase border focus:outline-none cursor-pointer ${getStatusBadge(
                            inv.status
                          )}`}
                        >
                          <option value="draft">Draft</option>
                        ...
                        {!isPaid && (
                          <button
                            type="button"
                            onClick={e => handleMarkAsPaid(e, inv)}
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold shadow-xs active:scale-95 transition-all cursor-pointer"
                    ...
                    {/* Due Date */}
                    <td className="py-3.5 px-4 text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-400 whitespace-nowrap">
                      {inv.dueDate}
                    </td>

                    {/* Amount / Budget */}
                    <td className="py-3.5 px-4 font-mono font-black text-slate-900 dark:text-white text-sm sm:text-base text-right whitespace-nowrap">
                      {inv.currency || '$'}{getInvTotal(inv).toLocaleString()}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right whitespace-nowrap">
                      ...
                      >
                        ...
                        <button
                          onClick={e => handleDownloadPdf(e, inv)}
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 transition-colors cursor-pointer"
